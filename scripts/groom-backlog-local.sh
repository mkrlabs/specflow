...
Steps:
1. Run `.claude/skills/board/scripts/list.sh --repo specnaut Backlog` to list current Backlog items.
2. For each item, run `.claude/skills/board/scripts/view.sh --repo specnaut <num>` to read the issue body and existing comments.
