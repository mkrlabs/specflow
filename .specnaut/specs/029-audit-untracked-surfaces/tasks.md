...
- [ ] T001 Confirm the working tree is clean and the branch is
...
- [ ] T002 Hoist the git invocation prefix into one
...
- [ ] T003 Hoist the four surface prefixes into one
...
- [ ] T004 Assert the toplevel invariant at the `--src-root` flag validation in
...
- [ ] T005 Verify T002–T004 changed nothing observable:
...
- [ ] T006 [US1] Add the untracked collection to `scripts/smoke/audit.sh`:
...
- [ ] T007 [US1] Add the staged collection to `scripts/smoke/audit.sh`:
...
- [ ] T008 [US1] De-duplicate the three-source union with `awk '!seen[$0]++'` — first-seen order
...
- [ ] T009 [US1] Mark a path that came from the untracked source with `(untracked)` in the report
...
- [ ] T010 [US1] Write the rationale as a comment above the union in `scripts/smoke/audit.sh`,
...
- [ ] T011 [US1] Add the US1 assertions to `scripts/smoke/smoke-audit.sh`: plant an untracked
...
- [ ] T012 [US1] Extend `smoke-audit.sh`'s existing non-ASCII scenario (3h) to plant an
      **untracked** accented surface file, so `-c core.quotePath=false` has a witness on the new
      ...
      #549.
- [ ] T013 [US1] **Observe red**: delete the `ls-files` line from `audit.sh`, run `smoke-audit.sh`,
...
- [ ] T014 [P] [US2] Add a `.gitignore` to `smoke-audit.sh`'s synthetic repo root carrying `*.log` —
      ...
      existing count.
- [ ] T015 [US2] Plant `templates/core/agents/debug.log` — under a surface prefix, matched by the
...
- [ ] T016 [US2] Add the separate pathspec assertion: a planted untracked file **outside** the four
...
- [ ] T020 [P] Report a collected path ending in `/` — an untracked directory containing its own
...
- [ ] T021 [P] Assert T020 in `smoke-audit.sh` by planting a nested `git init` under
...
- [ ] T022 [P] Assert FR-014 in `smoke-audit.sh`: plant a `chmod 000` untracked surface file and
...
- [ ] T023 Update `scripts/smoke/README.md` "Audit heuristics": state that the changed set is
...
- [ ] T024 Fix the **pre-existing** staleness in the same file: the Unmapped-surface row says "a
...
- [ ] T025 Add the comment beside `smoke-audit.sh`'s path literals saying they are a **deliberate
...
- [ ] T026 Run `deno fmt --check` separately — `deno task test` is `bundle && test` and does **not**
      run it.
- [ ] T027 Run the full battery: `deno task test`, then `bash scripts/smoke/run-all.sh` (which runs
...
- [ ] T028 Re-check the real tree after the full smoke run: `git status --short` and the four-prefix
...
- [ ] T029 Verify FR-007 mechanically: `git diff main..HEAD -- scripts/smoke/coverage-allowlist.txt`
      and the `SURFACES` region of `audit.sh` must both be empty of changes.
- [ ] T030 Commit with the `## Agent adoption` section and the rationale in the body (FR-008), then
